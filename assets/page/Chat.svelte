...
$: if (mainEl) q(mainEl, '.message', el => el.classList[el.dataset.ts == $route.hash ? 'add' : 'remove']('has-focus'));
...
afterUpdate(() => {
  ...
  if (firstTs && scrollSpy._chatFirstTs != firstTs) q(document, '.message__embed', embedEl => embedEl.remove());
  scrollSpy._chatFirstTs = firstTs;

  // Make sure we observe all message elements
  scrollSpy.wrapper = mainEl;
  scrollSpy.observe('.message');
  if (messagesHeight != scrollSpy._chatMessagesHeight) scrollSpy.keepPos(messagesHeight);
  scrollSpy._chatMessagesHeight = messagesHeight;
});
...
async function setDialogFromUser(user) {
  ...
  await dialog.load({after: after ? after.setSeconds(after.getSeconds() - 2).toISOString() : 'maybe'});
